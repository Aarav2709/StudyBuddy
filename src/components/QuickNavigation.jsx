      ...
      case 'fire':
        return (
          <svg className={iconClasses} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.879 16.121A3 3 0 1012.015 11L11 14l4-3a3 3 0 11-2.5 3z" />
          </svg>
        );
      case 'trophy':
        return (
  ...
  return (
    <>
      <button
        ...
        className={`fixed top-4 left-4 z-50 p-2 rounded-lg lg:hidden transition-colors focus:outline-none focus:ring-2 ${
          isDark 
            ? 'bg-gray-900 border border-gray-700 text-gray-400 hover:text-white focus:ring-gray-600' 
            : 'bg-white border border-gray-200 text-gray-600 hover:text-black focus:ring-gray-400'
        }`}
      >
      ...
      <div className={`fixed left-0 top-0 h-full z-50 transition-all duration-300 ${
        isNavOpen ? 'w-64' : 'w-16 -translate-x-full lg:translate-x-0'
      } ${isDark ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'} border-r lg:translate-x-0`}>
        <button
          onClick={() => setIsNavOpen(!isNavOpen)}
          className={`absolute -right-3 top-6 w-6 h-6 rounded-full border-2 items-center justify-center transition-colors hidden lg:flex ${
            isDark 
